...
- (void)insertFavoriteItemsAtBeginning:(NSArray *)items {
    ...
    for (SoundCloudItem *item in [items reverseObjectEnumerator]){
...
//                // this might need to be reversed if it brings in playlist tracks backwards
//                [self.favoriteItemsToShowInTableView insertObject:playlistTrack atIndex:0];
//            }
        }
    }
...
- (void)insertStreamItemsAtBeginning:(NSArray *)items {
    SoundCloudItem *lastItem = [items lastObject];
        ...
        for (SoundCloudItem *item in [items reverseObjectEnumerator]){
            if (item.type == SoundCloudItemTypeTrack || item.type == SoundCloudItemTypeTrackRepost) {
                ...
                if (trackForItem.playerItem) {
                    [self.streamItemsToShowInTableView insertObject:trackForItem atIndex:0];
                    if (itemsToPlay.count < 3) {
                        [itemsToPlay insertObject:trackForItem.playerItem atIndex:0];
                    }
                }
            } else if (item.type == SoundCloudItemTypePlaylist || item.type == SoundCloudItemTypePlaylistRepost){
                ...
                if (playlistForItem.streamable) {
                    // TODO: Playlists disabled
//                    [self.streamItemsToShowInTableView insertObject:playlistForItem atIndex:0];
//                    for (SoundCloudTrack *playlistTrack in playlistForItem.tracks) {
//                        [self.streamItemsToShowInTableView insertObject:playlistTrack atIndex:0];
//                        if (itemsToPlay.count < 3) {
//                            [itemsToPlay insertObject:playlistTrack.playerItem atIndex:0];
//                        }
//                    }
                }
            }
            ...
            if (item.type == SoundCloudItemTypeTrack || item.type == SoundCloudItemTypeTrackRepost) {
                ...
                if (trackForItem.playerItem) {
                    ...
                    if (self.itemsToPlay.count < 3) {
                        [self.itemsToPlay insertObject:trackForItem.playerItem atIndex:0];
                    }
                }
            } else if (item.type == SoundCloudItemTypePlaylist || item.type == SoundCloudItemTypePlaylistRepost){
                SoundCloudPlaylist *playlistForItem = item.item;
                if (playlistForItem.streamable) {
                    // TODO: Playlists disabled
//                    [self.streamItemsToShowInTableView insertObject:playlistForItem atIndex:0];
//                    for (SoundCloudTrack *playlistTrack in [playlistForItem.tracks reverseObjectEnumerator]) {
//                        [self.streamItemsToShowInTableView insertObject:playlistTrack atIndex:0];
//                        if (self.itemsToPlay.count < 3) {
//                            [self.itemsToPlay insertObject:playlistTrack.playerItem atIndex:0];
//                        }
//                    }
                }
            }
    ...
    if ([keyPath isEqualToString:@"status"]){
        if (self.audioPlayer.status == AVPlayerStatusReadyToPlay){
            [self.audioPlayer prerollAtRate:1.0 completionHandler:^(BOOL finished) {
                NSLog(@"Finished Prerolling: %@", finished ? @"YES" : @"NO");
            }];
        }
...
- (AVPlayerItem *)itemForDict:(NSDictionary *)dict {
    if ([dict[@"type"] isEqualToString:@"track"] && [dict[@"origin"][@"streamable"] boolValue] && [dict[@"origin"][@"state"] isEqualToString:@"finished"] && [dict[@"origin"][@"sharing"] isEqualToString:@"public"]) {
        ...
        streamURLString = [streamURLString stringByAppendingString:[NSString stringWithFormat:@"?client_id=%@&allow_redirects=False",[Keys clientID]]];
